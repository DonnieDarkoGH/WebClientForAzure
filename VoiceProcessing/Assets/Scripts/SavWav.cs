...
public static class SavWav {
	...
	static Byte[] ConvertAndWrite(FileStream fileStream, AudioClip clip) {
        ...
        //byteSamples = bytesData;
        //writer.Write(System.Text.Encoding.UTF32.GetChars(bytesData));
    ...
    static Byte[] WriteHeader(FileStream fileStream, AudioClip clip) {
        ...
        Byte[] alignByte = BitConverter.GetBytes(blockAlign);
        fileStream.Write(alignByte, 0, 2);
        ...
        Byte[][] byteArr = { riff, chunkSize, wave, fmt, subChunk1, audioFormat, audioFormat, numChannels, sampleRate, byteRate, alignByte, bitsPerSample, datastring, subChunk2 };

        int len = 0;
        for (int i =0; i < byteArr.GetLength(0); i++)
        {
            len += byteArr[i].Length;
        }
        ...
        for (int i = 0; i < byteArr.GetLength(0); i++)
        {
            byteArr[i].CopyTo(byteArray, index);
            index += byteArr[i].Length;
        }
